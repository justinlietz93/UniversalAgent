...
class Config:
    ...
    def __init__(self):
        ...
        if not self.AGENT_API_KEY:
            raise ValueError("AGENT_API_KEY environment variable must be set")
    ...
    def validate_api_keys(cls) -> None:
        ...
        Raises:
            ValueError: If the required Agent API key is missing
        """
        if not cls.AGENT_API_KEY:
            raise ValueError(
                "Missing required API key: AGENT_API_KEY. "
