...
class FileTool(Tool):
    ...
    def description(self) -> str:
        return (
            ...
            "- list_dir: List contents of a directory (requires path, optional recursive)\n"
        )
    ...
    def input_schema(self) -> Dict[str, Any]:
        return {
            ...
            "properties": {
                ...
                "path": {
                    "type": "string", 
                    "description": "Path to the file or directory to operate on (relative to CODEBASE_PATH)"
                },
                "content": {
                    "type": "string",
    ...
    def run(self, input: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Prepend CODEBASE_PATH to the relative path
        codebase_path = os.getenv("CODEBASE_PATH", "C:\\git\\my_project")
        path = os.path.join(codebase_path, relative_path) if relative_path else codebase_path

        # Define file-specific operations that should have a .py extension if none is present
        file_operations = ["read", "write", "read_lines", "edit_lines", "append"]
        # Check if the path already has an extension
        has_extension = os.path.splitext(path)[1] != ""
        # Append .py only for file operations if no extension is present
        if operation in file_operations and not has_extension:
            path += ".py"

        path = str(Path(path).absolute())
        print(f"Accessing path: {path}")  # Debug print
